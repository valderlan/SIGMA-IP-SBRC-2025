...
def remove_rules_for_custom_chains():
    for chain in ["INPUT", "FORWARD"]:
        ...
        rules = run_iptables_command(
            f"sudo iptables -L {chain} --line-numbers -n", capture_output=True
        )
...
def create_or_flush_chain(chain_name):
    subprocess.run(
        f"sudo iptables -N {chain_name} || sudo iptables -F {chain_name}", shell=True
    )
...
def setup_blacklist_chain():
    ...
    run_iptables_command(
        "sudo iptables -A BLACKLIST -p tcp -j REJECT --reject-with tcp-reset"
    )
    run_iptables_command("sudo iptables -A BLACKLIST -j DROP")
...
def apply_rules_to_ips():
    try:
        ...
        for ip in ips:
            ...
            for chain in ["FORWARD", "INPUT"]:
                run_iptables_command(
                    f"sudo iptables -I {chain} -s {ip[0]} -j BLACKLIST"
                )
            print(
                f"Regras de BLACKLIST aplicadas para o IP {ip[0]} em FORWARD e INPUT."
            )
