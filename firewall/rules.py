...
def create_or_flush_chain(chain_name):
    ...
    subprocess.run(
        f"sudo iptables -N {chain_name} 2>/dev/null || sudo iptables -F {chain_name}",
        shell=True,
    )
...
def setup_whitelist_chain():
    ...
    run_iptables_command("sudo iptables -A WHITELIST -j ACCEPT")
    ...
    try:
        ...
        for ip in ips:
            ...
            for chain in ["FORWARD", "INPUT"]:
                run_iptables_command(
                    f"sudo iptables -I {chain} -s {ip[0]} -j WHITELIST"
                )
            print(
                f"Regras de WHITELIST aplicadas para o IP {ip[0]} em FORWARD e INPUT."
            )
